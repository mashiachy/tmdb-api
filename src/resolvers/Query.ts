...
> = async (_, args, { dataSources }, info) =>
  dataSources.TMDB.collection(args, info)
...
> = setLanguage(async (_, { ids, ...args }, context, info) =>
  Promise.all(
    ids.map(async (id) =>
      context.dataSources.TMDB.person({ id, ...args }, info)
    )
  )
)
...
export const reviews: Resolver<
  {},
  ByIDList,
  Promise<Model<"review">[]>
> = async (_, { ids, ...args }, { dataSources }, info) =>
  Promise.all(
    ids.map(async (id) => dataSources.TMDB.review({ id, ...args }, info))
  )
...
export const trending: Resolver<
  ...
  Promise<Model<"movie" | "person" | "tv">[]>
> = async (_, args, { dataSources }, info) =>
  dataSources.TMDB.trending(args, info)
